...
from app.config import QUOTES_DIR, BusinessConfig, ensure_dirs
...
def render_quote_pdf(
    ...
    out_path = QUOTES_DIR / f"quote-{quote_id:05d}.pdf"
    ...
    if business.vat_pct:
        rows.append([
            Paragraph(rtl(_money(breakdown.subtotal, breakdown.currency_symbol)),
                      tot_label),
            Paragraph(rtl('סכום לפני מע"מ'), tot_label),
        ])
        rows.append([
            Paragraph(rtl(_money(breakdown.vat_amount, breakdown.currency_symbol)),
                      tot_label),
            Paragraph(rtl(f'מע"מ {business.vat_pct:g}%'), tot_label),
        ])
    rows.append([
        Paragraph(rtl(_money(breakdown.total, breakdown.currency_symbol)), tot_bold),
        Paragraph(rtl('סה"כ לתשלום'), tot_bold),
    ])
